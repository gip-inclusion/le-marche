"""Original from here https://github.com/robertkovac/django-fieldsets-with-inlines but not maintained anymore"""

from django import forms


class FieldsetsInlineMixin:
    change_form_template = "admin_fieldsets/change_form.html"
    ...
    def get_fieldsets(self, request, obj=None):
        if self.fieldsets_with_inlines:
            return [
                self.make_placeholder(index, fieldset) for index, fieldset in enumerate(self.fieldsets_with_inlines)
            ]
    ...
    def get_inline_instances(self, request, obj=None):
        ...
        if self.fieldsets_with_inlines:
            inlines = [
                inline for inline in self.fieldsets_with_inlines if isinstance(inline, forms.MediaDefiningClass)
            ]
            inline_instances = []
