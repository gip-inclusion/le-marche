...
class TenderCreateApiTest(TestCase):
    ...
    def test_create_tender_with_tally_source(self, mock_add_to_contact_list):
        ...
        # Verify that `tender_id` is not None
        self.assertIsNotNone(kwargs.get("tender_id"), "Expected tender_id to be assigned and not None.")
    ...
    def test_create_contact_call_has_user_buyer_attributes(self, mock_create_contact):
        ...
        _, tender, user = self.setup_mock_user_and_tender_creation(title="Test tally", user=self.user_buyer)
        ...
        self.assertEqual(attributes["TYPE_VERTICALE_ACHETEUR"], [sector.name for sector in tender.sectors.all()])
