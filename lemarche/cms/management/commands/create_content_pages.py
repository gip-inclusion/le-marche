...
class Command(BaseCommand):
    help = """
    Créer des pages de type ContentPage à partir d'un fichier JSON.
    """
    ...
    def handle(self, *args, **kwargs):
        try:
            ...
            self.stdout.write(self.style.ERROR("Le fichier lemarche/fixtures/cms_content_pages.json n'existe pas."))
            return
        except json.JSONDecodeError:
            self.stdout.write(
                self.style.ERROR(
                    "Le fichier lemarche/fixtures/cms_content_pages.json n'est pas un fichier JSON valide."
                )
            )
            return
    ...
    def create_content_page(self, slug: str, title: str, body: list, parent_page: ContentPage) -> ContentPage:
        ...
        if already_exists:
            self.stdout.write(f"La page avec le slug '{slug}' semble déjà exister avec l'id {already_exists.id}")
            return already_exists

        new_page = parent_page.add_child(instance=ContentPage(title=title, body=body, slug=slug, show_in_menus=True))

        self.stdout.write(self.style.SUCCESS(f"Page '{slug}' creée avec l'id {new_page.id}"))
