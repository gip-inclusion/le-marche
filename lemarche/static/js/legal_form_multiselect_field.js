document.addEventListener('DOMContentLoaded', function () {
    /**
     * Multiselect dropdown for the legal_form search form field
     */

    const FORM_INPUT_ID = "id_legal_form";
    ...
    const FORM_PLACEHOLDER = 'Association, SARL';
    ...
    const buttonTextAndTitle = function (options, select) {
        if (options.length === 0) {
            ...
            return `${options.length} formes sélectionnées`;
        }
        else {
            var labels = [];
