...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if options["dry_run"]:
            self.stdout.write(
                f"Dry-run: suppression des besoins d'achat inachevés: {qs.count()} auraient été supprimés"
            )
            return
        ...
        log = f"Suppression des besoins d'achat inachevés: {deleted.get('tenders.Tender')} ont été supprimés"
        if deleted:
            log += f" ({deleted})"
