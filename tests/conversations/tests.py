...
from tests.conversations.factories import ConversationFactory, EmailGroupFactory, TemplateTransactionalFactory
...
class ConversationAnonymizationTestCase(TestCase):
    ...
    def setUp(self):
        ...
        ConversationFactory(
            title="anonymized",
            created_at=datetime(year=2023, month=6, day=1, tzinfo=UTC),
            initial_body_message="blabla",
            data=[inbound_data, inbound_data],
        )
        ConversationFactory(created_at=datetime(year=2023, month=8, day=1, tzinfo=UTC))
