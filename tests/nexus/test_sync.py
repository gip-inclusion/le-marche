...
class TestUserSync:
    def test_sync_on_model_save(self, nexus_sync, mock_nexus_api):
        ...
        assert json.loads(call.request.content.decode()) == [
            {
                "id": str(user.pk),
                "kind": "SIAE",
    ...
    def test_delete_on_model_delete(self, nexus_sync, mock_nexus_api):
        ...
        assert call.request.url == "http://nexus/api/users"
        assert json.loads(call.request.content.decode()) == [{"id": str(user_id)}]
    ...
    def test_delete_on_manager_update(self, nexus_sync, mock_nexus_api):
        ...
        assert call.request.method == "DELETE"
        assert call.request.url == "http://nexus/api/users"
        assert json.loads(call.request.content.decode()) == [{"id": str(user_1.pk)}, {"id": str(user_2.pk)}]
...
class TestSiaeSync:
    ...
    def test_delete_on_model_delete(self, nexus_sync, mock_nexus_api):
        ...
        assert call.request.method == "DELETE"
        assert call.request.url == "http://nexus/api/structures"
        assert json.loads(call.request.content.decode()) == [{"id": str(siae_id)}]
    ...
    def test_delete_on_manager_update(self, nexus_sync, mock_nexus_api):
        ...
        assert call.request.method == "DELETE"
        assert call.request.url == "http://nexus/api/structures"
        assert json.loads(call.request.content.decode()) == [{"id": str(siae_1.pk)}, {"id": str(siae_2.pk)}]

        mock_nexus_api.reset()
        Siae.objects.order_by("pk").update(is_delisted=False)
        [call] = mock_nexus_api.calls
        assert call.request.method == "DELETE"
        assert call.request.url == "http://nexus/api/structures"
        assert json.loads(call.request.content.decode()) == [{"id": str(siae_1.pk)}, {"id": str(siae_2.pk)}]
...
class TestSiaeUserSync:
    def test_sync_on_model_save(self, nexus_sync, mock_nexus_api):
        siae_user = SiaeUserFactory.build()
        siae_user.user.save()
        siae_user.siae.save()
        mock_nexus_api.reset()
    ...
    def test_delete_on_model_save(self, nexus_sync, mock_nexus_api):
        ...
        assert call.request.method == "DELETE"
        assert call.request.url == "http://nexus/api/memberships"
        assert json.loads(call.request.content.decode()) == [{"id": str(siae_user.pk)}]

    def test_sync_on_model_delete(self, nexus_sync, mock_nexus_api):
        siae_user = SiaeUserFactory()
        mock_nexus_api.reset()
        ...
        assert call.request.method == "DELETE"
        assert call.request.url == "http://nexus/api/memberships"
        assert json.loads(call.request.content.decode()) == [{"id": str(siae_user_id)}]
    ...
    def test_delete_on_manager_delete(self, nexus_sync, mock_nexus_api):
        ...
        assert call.request.method == "DELETE"
        assert call.request.url == "http://nexus/api/memberships"
        assert json.loads(call.request.content.decode()) == [{"id": str(siae_user_1.pk)}, {"id": str(siae_user_2.pk)}]
